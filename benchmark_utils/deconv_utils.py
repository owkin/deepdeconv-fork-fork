...
def initialize_deconv_methods(
    ...
    for deconv_method in deconv_methods:
        ...
        if (deconv_method in MODEL_TO_FIT) == (
            deconv_method in SIGNATURE_MATRIX_MODELS
        ):
            message = (
        ...
        if deconv_method in MODEL_TO_FIT:
            ...
            if "adata_pseudobulk" in kwargs:
                ...
                kwargs["adata_pseudobulk"] = train_pseudobulks[
...
def use_nnls_method(to_deconvolve: pd.DataFrame, signature_matrix: pd.DataFrame):
    ...
    # Run NNLS
    deconv = LinearRegression(positive=True).fit(signature_matrix, to_deconvolve)
...
def save_deconvolution_results(deconv_results: dict, experiment_path):
    """Save the deconvolution results in the experiment path.

    Parameters
    ----------
    deconv_results : dict
        The deconvolution results to save
    experiment_path : str
        The path to save the deconvolution results
    """
...
def create_random_proportion(
    n_classes: int, n_non_zero: int | None = None
) -> np.ndarray:
