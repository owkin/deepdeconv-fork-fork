...
cv_sc = single_cell_data[common_genes].values.std(axis=0) / single_cell_data[common_genes].values.mean(axis=0)
cv_bulk = bulk_facs_data[common_genes].values.std(axis=0) / bulk_facs_data[common_genes].values.mean(axis=0)
cv_bulk_tpm = bulk_facs_tpm_data[common_genes].values.std(axis=0) / bulk_facs_tpm_data[common_genes].values.mean(axis=0)
...
for key in tqdm(pseudobulk_data):
    cv_pseudobulk[key] = pseudobulk_data[key][common_genes].values.std(axis=0) / pseudobulk_data[key][common_genes].values.mean(axis=0)

cv_pseudobulk_sorted = {k: pd.Series(v, index=common_genes).sort_values() for k, v in cv_pseudobulk.items()}
# %%
# --- Calculating cumulative overlaps between CV-sorted genes ---
overlap_sc_bulk, n_genes_sc_bulk = calculate_cumulative_overlaps(cv_sorted_sc.index, cv_sorted_bulk.index)
overlap_sc_bulk_tpm, n_genes_sc_bulk_tpm = calculate_cumulative_overlaps(cv_sorted_sc.index, cv_sorted_bulk_tpm.index)
...
for key in cv_pseudobulk_sorted:
    overlap_pseudobulk, n_genes_pseudobulk = calculate_cumulative_overlaps(cv_pseudobulk_sorted[key].index, cv_sorted_bulk.index)
    overlap_pseudobulk_tpm, n_genes_pseudobulk_tpm = calculate_cumulative_overlaps(cv_pseudobulk_sorted[key].index, cv_sorted_bulk_tpm.index)
...
plt.plot(n_genes_sc_bulk, percentages(overlap_sc_bulk, n_genes_sc_bulk), 
         label='Single Cell vs Bulk FACS')
plt.plot(n_genes_sc_bulk_tpm, percentages(overlap_sc_bulk_tpm, n_genes_sc_bulk_tpm),
         label='Single Cell vs Bulk FACS TPM')
for i, key in enumerate(cv_pseudobulk_sorted):
    plt.plot(n_genes_pseudobulk_list[i], percentages(overlap_pseudobulk_list[i], n_genes_pseudobulk_list[i]),
             label=f'{key} vs Bulk FACS')
    plt.plot(n_genes_pseudobulk_tpm_list[i], percentages(overlap_pseudobulk_tpm_list[i], n_genes_pseudobulk_tpm_list[i]),
             label=f'{key} vs Bulk FACS TPM', linestyle='--')
    
plt.hlines(y=1.0, xmin=0, xmax=len(n_genes_sc_bulk), color='g', linestyle='--', alpha=0.8, label='Perfect overlap (100%)')
plt.vlines(x=0, ymin=0, ymax=1, color='g', linestyle='--', alpha=0.8)
...
for i, key in enumerate(cv_pseudobulk_sorted):
    ...
    fig.add_trace(go.Scatter(
        ...
        y=percentages(overlap_pseudobulk_tpm_list[i], n_genes_pseudobulk_tpm_list[i]),
...
dirichlet_100_sum_genes = list(cv_pseudobulk_sorted['dirichlet_pb_100_sum'][:2000].index)
...
def between_within_fraction(adata, gene_list, cell_type_key):
    ...
    group_codes, groups = pd.factorize(adata.obs[cell_type_key].values,
                                       sort=True)      # reproducible order
    K = len(groups)
    ...
    indicator = sparse.csr_matrix((np.ones(n_cells, dtype=np.float32),
                                   group_codes, indptr),
                                   shape=(n_cells, K))
    ...
    ss_within     = (group_sq_sums
                     - n_k[:, None] * group_means**2).sum(axis=0)
...
with open('project/filtered_genes_bw_ratio_sc.pkl', 'wb') as f:
    ...
    print(f"Saved top genes to project/filtered_genes_bw_ratio_sc.pkl")
